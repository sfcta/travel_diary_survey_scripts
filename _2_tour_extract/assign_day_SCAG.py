...
raw_per = pd.read_csv(join(RAW_DIR, 'ex_person.tsv'), sep='\t')
...
raw_trips = pd.read_csv(join(RAW_DIR, 'ex_trip.tsv'), sep='\t')
...
raw_trips.loc[pd.isna(raw_trips['dow']), 'dow'] = pd.to_datetime(raw_trips.loc[pd.isna(raw_trips['dow']), 'travel_date']).dt.dayofweek + 1
raw_trips = raw_trips.drop('travel_date', axis=1)
