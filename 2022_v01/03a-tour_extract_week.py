...
def tour_extract_week(config):
    ...
    if weighted:
        ...
        tripn = np.empty((PMAX, TMAX), dtype=int)
        ...
        tday = np.empty((PMAX, TMAX), dtype=int)
        ...
        for h in range(len(hh)):
            ...
            for p in range(1, len(hpers) + 1):
                ...
                for t in range(1, len(hpertrips) + 1):
                    tripn[p, t] = hpertrips["tripno"][t - 1]
                    tsvid[p, t] = hpertrips["tsvid"][t - 1]
                    ...
                    tday[p, t] = hpertrips["day"][t - 1] # new
                    ...
                    if hhno == 23251383:
                        if t >=20 and t <= 24:
                            print('trip: {}, dpurp: {}, isclosex: {} ({}, {})'.format(t, tdprp[p, t], isclose(tdxco[p, t], hhxco), tdxco[p, t], hhxco))
                    if tdprp[p, t] == 0 or (
                    ...
                    if (tday[p, t] - tday[p, t - 1] > 1) or t == 1: # replace with check on dow with day, which is the actual sequence of travel days
            ...
            for p in range(1, len(hpers) + 1):
                # tour formation logic - build weeklong tours
                d = 0
                ...
                if t2 > 0:
                    ...
                    for t in range(1, t2 + 1):
                        ...
                        # last trip of tour returning home
                            ...
                            if (
                                ...
                                #if hhno == 23251383:
                                #    print('trip: {}, day: {}, first-of-day: {}'.format(t, tday[p, t], fday))
                                #if tdow[p, t] - fdow > 1:
                                if tday[p, t] - fday > 1:
                                    fact = 0
                                    continue
                    ...
                    if hhno == 23251383:
                        for _t in range(hbtour[p, d] + 1):
                            print('tour: {}, leave origin: {}, arrive origin: {}'.format(_t, leaveorig[p, d, _t],arrivorig[p, d, _t]))
                        input('continue')
