...
class SkimUtil2:
    ...
    def __init__(self, skimdir, timeperiods=[2], skimprefix=""):
        ...
        for tkey in timeperiods:
            self.hwyskims[tkey] = open_file(
                os.path.join(
                    skimdir, skimprefix + "HWYALL" + TIMEPERIODS[tkey] + ".h5"
                ),
                mode="r",
            )
    ...
    def getDASkims(self, otaz, dtaz, timeperiod=2):
        ...
        if (
            otaz >= self.termtime.get_node("/", "1").shape[0]
            or dtaz >= self.termtime.get_node("/", "1").shape[0]
        ):
            termtime = 0
        else:
